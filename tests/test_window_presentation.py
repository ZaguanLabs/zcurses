...
class WindowPresentationTests(unittest.TestCase):
    def test_geometry_changes_require_presentation(self):
        features = test_features.FeatureTests().run_shell(
            'zmodload zdraw || exit 1; print -rl -- "${zdraw_features[@]}"')
        if not {'norefresh_events', 'window_resize', 'window_movement'}.issubset(features.splitlines()):
        ...
        if pid == 0:
            ...
            os.execl(ZSH, ZSH, '-df', str(ROOT / 'tests/window-presentation.zsh'),
        ...
        try:
            while True:
                ...
                if step == b'baseline':
                    ...
                    self.assertNotIn(b'WINDOWSECRET', screen)
                elif step == b'presented':
                    self.assertIn(b'WINDOWSECRET', screen)
                elif step == b'changed':
                    self.assertNotIn(b'LARGERWINDOW', screen)
                elif step == b'changedpresented':
                    self.assertIn(b'LARGERWINDOW', screen)
                else:
            ...
            self.assertEqual(steps, [b'baseline', b'hidden', b'presented',
                                     b'changed', b'changedpresented'])
        finally:
            if not reaped:
                try:
