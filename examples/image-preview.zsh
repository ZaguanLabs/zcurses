...
image_packet=$(command python3 "$image_root/scripts/image-preview.py" --rows 24 --columns 80 -- "$1")
image_status=$?
...
function image-render {
  emulate -L zsh
  local -i image_rows image_columns image_h image_w
  zdraw position stdscr image_position || return
  image_rows=$image_position[5] image_columns=$image_position[6]
  zdraw fill stdscr 0 0 "$image_rows" "$image_columns" '' ' ' || return
  zdraw-label stdscr 0 0 "$image_columns" "$image_name / $image_palette / $image_colors" normal fg=accent bold || return
  if (( image_status )); then
    ...
    image_h=$((image_rows-4 < 24 ? image_rows-4 : 24)) image_w=$((image_columns < 80 ? image_columns : 80))
    zdraw-image-draw stdscr 2 0 "$image_h" "$image_w" normal "palette=$image_palette" "colors=$image_colors" || return
  fi
  (( image_rows > 1 )) && zdraw-label stdscr "$((image_rows-1))" 0 "$image_columns" 'g glyphs / m monochrome / s suspend-resume / q quit' normal fg=muted
...
zdraw init || exit 1
{
  ...
  if (( ! image_exit )); then
    if (( image_info[colors] >= 16 )) && [[ -z ${NO_COLOR:-} ]]; then zdraw-ui-theme dark 16
