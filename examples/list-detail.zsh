...
# Run from the checkout with .build/zsh/Src/zsh -df examples/list-detail.zsh
emulate -R zsh
setopt nounset
...
typeset -A zdraw_ui_theme zdraw_ui_list event colors
...
typeset theme_name=dark profile=mono view=list layout_mode=tiny
...
function recipe-render {
  ...
  local -a frame body footer list_frame detail_frame content lines
  local -i selected i
  ...
  frame=("${reply[@]}")
  zdraw-label stdscr 0 "$frame[2]" "$frame[4]" 'PROJECTS / a little room to think' normal bg=canvas fg=accent bold || return
  ...
  footer=("${reply[@]}")
  list_frame=("${body[@]}") detail_frame=("${body[@]}")
  layout_mode=single
  if (( body[4] >= 72 )); then
    layout_mode=split
  ...
  if [[ $layout_mode == split || $view == list ]]; then
    zdraw-panel stdscr "${list_frame[@]}" ' Collection ' focus border=rounded px=1 title:fg=accent || return
    content=("${reply[@]}") visible=$reply[3]
    zdraw-list-update "${#projects}" "$visible" keep || return
    if (( content[3] && content[4] )); then
      zdraw-list stdscr "${content[@]}" focus -- "${projects[@]}" || return
    fi
  else
    zdraw-list-update "${#projects}" 0 keep || return
  fi
  if [[ $layout_mode == split || $view == detail ]]; then
    selected=$zdraw_ui_list[selected]
    zdraw-panel stdscr "${detail_frame[@]}" ' Project notes ' normal border=rounded px=1 py=0 title:fg=accent || return
    content=("${reply[@]}")
    lines=("$projects[$selected]" "$stages[$selected] / $owners[$selected]" '')
    ...
    lines+=('' 'Make space for the next good idea.')
  ...
  local hint='j/k select  Enter details/back  t theme'
  [[ $layout_mode == split ]] && hint='j/k select  Home/End  PgUp/PgDn  t theme'
  zdraw-label stdscr "$footer[1]" "$footer[2]" "$footer[4]" "$hint" normal bg=canvas fg=muted || return
  zdraw-label stdscr "$((footer[1]+1))" "$footer[2]" "$footer[4]" 'q quit  Esc back/quit' normal bg=canvas || return
  zdraw refresh stdscr
}
...
zdraw init || exit 1
{
  ...
  zdraw-list-update "${#projects}" 0 keep || exit 1
  while true; do
    if (( dirty )); then
    ...
    if zdraw event stdscr event "${input_options[@]}"; then
      ...
      case $event[type] in
        character)
          case $event[text] in
            q) break ;;
            $'\e')
              if [[ $layout_mode == single && $view == detail ]]; then view=list; else break; fi ;;
            $'\n'|$'\r')
              if [[ $layout_mode == single ]]; then
                if [[ $view == list ]]; then view=detail; else view=list; fi
              fi ;;
      ...
      zdraw-list-update "${#projects}" "$visible" "$action" || { exit_code=1; break; }
      dirty=1
    fi
  done
} always {
