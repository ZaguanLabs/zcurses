...
function zdraw-tabs {
  emulate -L zsh
  (( $# >= 7 )) || { _zdraw_ui_error 1 "${(%):-%N}" "expected window row column width selected state [utilities ...] -- [labels ...]"; return $?; }
  [[ ${(t)zdraw_ui_theme} == association* ]] || { _zdraw_ui_error 1 "${(%):-%N}" "requires a zdraw_ui_theme association"; return $?; }
  _zdraw_ui_uint "$5" || { _zdraw_ui_error 1 "${(%):-%N}" "tab selection must be an integer from 0 to 32767"; return $?; }
  ...
  [[ $_zui_states == (focus|inactive|disabled) ]] || { _zdraw_ui_error 1 "${(%):-%N}" "tabs state must be focus, inactive or disabled"; return $?; }
  _zdraw_ui_rect "$1" "$2" "$3" 1 "$4" || return
  shift 6
  while (( $# )) && [[ $1 != -- ]]; do _zui_tokens+=("$1"); shift; done
  (( $# )) || { _zdraw_ui_error 1 "${(%):-%N}" "missing -- before tab labels"; return $?; }
  shift
  _zui_items=("$@")
  (( $# <= 32 && (($# == 0 && _zui_selected == 0) || (_zui_selected >= 1 && _zui_selected <= $#)) )) || { _zdraw_ui_error 1 "${(%):-%N}" "tabs require at most 32 labels and a valid selection (0 only for empty tabs)"; return $?; }
  ...
  for _zui_part in normal selected; do
    ...
    [[ $zdraw_ui_style[border] == none && $zdraw_ui_style[py] == 0 ]] || { _zdraw_ui_error 1 "${(%):-%N}" "tabs require border=none and py=0"; return $?; }
  ...
  for (( _zui_i=1; _zui_i<=${#_zui_items}; _zui_i++ )); do
    ...
    (( _zui_total <= 262144 )) || { _zdraw_ui_error 1 "${(%):-%N}" "tab labels exceed 262144 characters"; return $?; }
  ...
  zdraw fill "$_zui_win" "$_zui_y" "$_zui_x" 1 "$_zui_w" "$_zui_styles[normal,style]" ' ' || { _zdraw_ui_error $? "${(%):-%N}" "native fill failed"; return $?; }
  ...
  for (( _zui_i=_zui_first; _zui_i<=_zui_last; _zui_i++ )); do
    ...
    zdraw fill "$_zui_win" "$_zui_y" "$((_zui_x+_zui_offset))" 1 "$_zui_width" "$_zui_styles[$_zui_part,style]" ' ' || { _zdraw_ui_error $? "${(%):-%N}" "native fill failed"; return $?; }
