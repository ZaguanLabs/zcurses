...
function zdraw-form-init {
  emulate -L zsh
  [[ ${(t)zdraw_ui_form} == (association|association-local) ]] || { _zdraw_ui_error 1 "${(%):-%N}" "requires a writable zdraw_ui_form association"; return $?; }
  (( $# > 0 && $# <= 48 && $# % 3 == 0 )) || { _zdraw_ui_error 1 "${(%):-%N}" "expected 1 to 16 label/text/rules triples"; return $?; }
  ...
  while (( $# )); do
    (( _zui_i++ ))
    (( ${#1} <= 128 )) || { _zdraw_ui_error 1 "${(%):-%N}" "form field $_zui_i label exceeds 128 characters"; return $?; }
    zdraw textinfo _zui_info "$1" || { _zdraw_ui_error $? "${(%):-%N}" "native textinfo failed"; return $?; }
    zdraw-input-init "$2" || return
    _zui_rules=()
    [[ -n $3 ]] && _zui_rules=("${(@s:,:)3}")
    zdraw-input-check "${_zui_rules[@]}"
    _zui_result=$?
    (( _zui_result < 2 )) || { _zdraw_ui_error 1 "${(%):-%N}" "form field $_zui_i has invalid input state or rules"; return $?; }
...
function _zdraw_ui_form_state {
  emulate -L zsh
  [[ ${(t)zdraw_ui_form} == association* ]] || { _zdraw_ui_error 1 "${(%):-%N}" "requires a zdraw_ui_form association"; return $?; }
  _zdraw_ui_uint "${zdraw_ui_form[count]-}" && _zdraw_ui_uint "${zdraw_ui_form[focus]-}" || { _zdraw_ui_error 1 "${(%):-%N}" "form count and focus must be integers from 0 to 32767"; return $?; }
  (( 10#$zdraw_ui_form[count] >= 1 && 10#$zdraw_ui_form[count] <= 16 &&
     10#$zdraw_ui_form[focus] >= 1 && 10#$zdraw_ui_form[focus] <= 10#$zdraw_ui_form[count] )) || { _zdraw_ui_error 1 "${(%):-%N}" "form count must be 1 to 16 and focus must identify a field"; return $?; }
}
...
function zdraw-form-action {
  emulate -L zsh
  [[ $# -ge 1 && ${(t)zdraw_ui_form} == (association|association-local) ]] || { _zdraw_ui_error 2 "${(%):-%N}" "requires a writable zdraw_ui_form association and an action"; return $?; }
  _zdraw_ui_form_state || { _zdraw_ui_error 2 "${(%):-%N}" "form state is invalid for an action"; return $?; }
  ...
  case $1 in
    ...
    next|previous|validate)
      (( $# == 1 )) || { _zdraw_ui_error 2 "${(%):-%N}" "next, previous and validate take no arguments"; return $?; }
      # A form cannot transfer an unfinished paste to another field.
      [[ $zdraw_ui_input[paste_active] == 0 ]] || { _zdraw_ui_error 2 "${(%):-%N}" "finish or cancel the field paste before navigation or validation"; return $?; }
      for (( _zui_i=1; _zui_i<=10#$zdraw_ui_form[count]; _zui_i++ )); do
        ...
        zdraw-input-check "${_zui_rules[@]}"
        _zui_result=$?
        (( _zui_result < 2 )) || { _zdraw_ui_error 2 "${(%):-%N}" "form field $_zui_i has invalid input state or rules"; return $?; }
    ...
    *) { _zdraw_ui_error 2 "${(%):-%N}" "unknown form action"; return $?; } ;;
  esac
}
...
function zdraw-form {
  emulate -L zsh
  (( $# >= 6 )) || { _zdraw_ui_error 1 "${(%):-%N}" "expected window row column height width state [utilities ...]"; return $?; }
  _zdraw_ui_form_state || return
  [[ $6 == (focus|inactive|disabled) ]] || { _zdraw_ui_error 1 "${(%):-%N}" "form state must be focus, inactive or disabled"; return $?; }
  ...
  (( _zui_h >= 3 )) || { _zdraw_ui_error 2 "${(%):-%N}" "form needs at least three rows for a field"; return $?; }
  ...
  for (( _zui_i=1; _zui_i<=10#$zdraw_ui_form[count]; _zui_i++ )); do
    ...
    zdraw textinfo _zui_info "${zdraw_ui_form[$_zui_i,label]-}" || { _zdraw_ui_error $? "${(%):-%N}" "native textinfo failed"; return $?; }
    zdraw textinfo _zui_info "${zdraw_ui_form[$_zui_i,error]-}" || { _zdraw_ui_error $? "${(%):-%N}" "native textinfo failed"; return $?; }
  done
  for _zui_field_state in focus inactive disabled focus,invalid inactive,invalid disabled,invalid; do
    for _zui_variant in "$_zui_field_state" "$_zui_field_state,selected" "$_zui_field_state,cursor"; do
      ...
      [[ $zdraw_ui_style[border] == none && $zdraw_ui_style[px] == 0 && $zdraw_ui_style[py] == 0 && $zdraw_ui_style[align] == left ]] || { _zdraw_ui_error 1 "${(%):-%N}" "form requires border=none, px=0, py=0 and align=left"; return $?; }
    done
  done
  zdraw-ui-style "$_zui_state" fg=text bg=surface "${@:7}" || return
  zdraw fill "$_zui_win" "$_zui_y" "$_zui_x" "$_zui_h" "$_zui_w" "$zdraw_ui_style[style]" ' ' || { _zdraw_ui_error $? "${(%):-%N}" "native fill failed"; return $?; }
