...
function _zdraw_ui_layout_rect {
  ...
  (( $# == 4 )) || return 1
  local _zui_number
  for _zui_number in "$@"; do _zdraw_ui_uint "$_zui_number" || return 1; done
  _zui_y=$((10#$1)) _zui_x=$((10#$2)) _zui_h=$((10#$3)) _zui_w=$((10#$4))
  (( _zui_y + _zui_h <= 32767 && _zui_x + _zui_w <= 32767 ))
}

function zdraw-layout-inset {
  emulate -L zsh
  [[ $# == 8 && ${(t)reply} == (array|array-local) ]] || return 1
  local -i _zui_y _zui_x _zui_h _zui_w _zui_top _zui_right _zui_bottom _zui_left
  local _zui_number
  _zdraw_ui_layout_rect "${@:1:4}" || return 1
  for _zui_number in "${@:5}"; do _zdraw_ui_uint "$_zui_number" || return 1; done
...
function zdraw-layout-center {
  emulate -L zsh
  [[ $# == 6 && ${(t)reply} == (array|array-local) ]] || return 1
...
function zdraw-layout-split {
  emulate -L zsh
  [[ $# -ge 7 && $# -le 38 && ${(t)zdraw_ui_layout} == (association|association-local) ]] || return 1
  ...
  local -A _zui_layout
  _zdraw_ui_layout_rect "${@:1:4}" || return 1
  [[ $_zui_axis == (rows|columns) ]] || return 1
  _zdraw_ui_uint "$6" || return 1
  _zui_gap=$((10#$6))
  _zui_extent=$_zui_w
  ...
  for _zui_spec in "$@"; do
    ...
    [[ $_zui_kind == (fixed|flex) ]] || return 1
    _zdraw_ui_uint "$_zui_raw" || return 1
    _zui_value=$((10#$_zui_raw))
    if [[ $_zui_kind == fixed ]]; then
      (( _zui_fixed += _zui_value ))
    else
      (( _zui_value > 0 )) || return 1
...
function zdraw-layout-rect {
  emulate -L zsh
  [[ $# == 1 && ${(t)reply} == (array|array-local) && ${(t)zdraw_ui_layout} == association* ]] || return 1
  _zdraw_ui_uint "$1" && _zdraw_ui_uint "${zdraw_ui_layout[count]-}" || return 1
  local -i _zui_index=$((10#$1)) _zui_count=$((10#$zdraw_ui_layout[count]))
  local -i _zui_y _zui_x _zui_h _zui_w
  (( _zui_count >= 1 && _zui_count <= 32 && _zui_index >= 1 && _zui_index <= _zui_count )) || return 1
